    ...
    if len(status_check) == 1:
        ...
        for file_path, score_details in compared_scores.items():
            try:
                ...
                print('New song: %s found! Adding to tracker...' % 
                      score_details[0])
                cell = False
            if cell:
    ...
    for song in existing_scores:
        ...
        if title in flattened_scores:
            best_score = max(score,
                         int(flattened_scores[
                             flattened_scores.index(title) + 1]))
        if best_score > score:
            print('New high score found for: %s | Score: %s' % 
                 (title, best_score))
            ...
            for file_path, song in new_scores.items():
                if song[0] == title:
                    new_scores[file_path] = [title, best_score, difficulty, 
                                             stars, accuracy, date]
        elif best_score <= score:
            print('New score not better than existing score for song: %s' % 
                  title)
            for file_path, song in new_scores.items():
                if song[0] == title:
